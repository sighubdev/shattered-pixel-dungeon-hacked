...
public class Hero extends Char {
    ...
    public static final int MAX_LEVEL = 69;

    public static final int STARTING_STR = 420000000;
    ...
    private int defenseSkill = 100000;
	...
	if (belongings.weapon != null) {
	    ...
	    return belongings.weapon.speedFactor( this ) / 2;
			
	} else {
	    return RingOfFuror.attackDelayMultiplier(this) / 2;
	}
    }
	...
	if (Dungeon.level.adjacent( pos, dst ) || pos == dst) {
		...
		if (heap.type == Type.LOCKED_CHEST) { Notes.add(new GoldenKey(Dungeon.depth)); }
		else if (heap.type == Type.CRYSTAL_CHEST) { Notes.add(new CrystalKey(Dungeon.depth)); }
	...
	if (Dungeon.level.adjacent( pos, doorCell )) {
	    ...
	    // Add the required key to the inventory.
	    if (door == Terrain.LOCKED_DOOR) {
		Notes.add(new IronKey(Dungeon.depth));
				
	    } else if (door == Terrain.LOCKED_EXIT) {
		Notes.add(new SkeletonKey(Dungeon.depth));
	    }
	...
	// You cannot actually die, but in case it somehow happens, automatically respawn.
	curAction = null;
